...
#include "p_nemesis.h"		// JEV nemesis: tactics note + observe payload + proposals
...
static int AI_Serialize (void)
{
    ...
    {
	if (n < OBSBUF) n += snprintf (obsbuf+n, OBSBUF-n, ",");
	n += NEM_Serialize (obsbuf + n, OBSBUF - n);
    }
		      ...
		      fixed_t tx, fixed_t ty, int focus, int fortics, int after)
{
	...
	if (id >= 1 && id <= aient_count)
	{
	    ...
	    {   // JEV nemesis: remember the tactic now in effect per type, for
		// death-time attribution ("died while executing a flank").
		mobj_t* m = aient[id-1].mo;
		if (m && m->health > 0)
		    NEM_NoteTactic (AI_TypeName (m->type), AI_OrderName (order));
	    }
	}
    }
}
...
static void AI_HandleLine (char* line, int client)
{
    ...
    // JEV nemesis:  nemesis propose=<type> tactic_weight=<order>:<delta> [bias=<cm>]
    // Weight-shift proposals from the learning bridge.  Every delta is clamped
    // inside NEM_Propose -- a buggy bridge cannot unbalance the table.
    if (!strncmp(line, "nemesis", 7))
    {
	char	type_s[24] = "";
	char*	kv;
	char	work[512];
	strncpy (work, line+7, sizeof(work)-1); work[sizeof(work)-1] = 0;
	for (kv = strtok(work, " \t\r\n"); kv; kv = strtok(NULL, " \t\r\n"))
	{
	    char* eq = strchr(kv, '='); if (!eq) continue; *eq++ = 0;
	    if      (!strcmp(kv,"propose"))	{ strncpy(type_s,eq,sizeof(type_s)-1); type_s[sizeof(type_s)-1]=0; }
	    else if (!strcmp(kv,"tactic_weight") && type_s[0])
	    {
		char key[64];
		snprintf (key, sizeof(key), "tactic_weight=%s", eq);
...
void P_AI_RuleTactics (void)
{
	...
	if (los[i])
	{
	    // JEV nemesis: per-type learned tactic weights steer the rule selection,
	    // so the offline/rule director still benefits from the learned table
	    // (zero model calls).
	    const char* tn = AI_TypeName (m->type);
	    d       = P_AproxDistance (m->x - pl->x, m->y - pl->y);
	    wounded = (m->info->spawnhealth > 0 && m->health * 4 <= m->info->spawnhealth);
	    if (wounded && d > TAC_MELEE*2 && NEM_TacticWeight (tn, "fallback") >= 0.5f)
...
void P_AI_Ticker (void)
{
    ...
    // JEV nemesis: tactic weights relax toward neutral over level time (~1/s
    // decay pass), so the AI can't stay maxed forever but a session's learning
