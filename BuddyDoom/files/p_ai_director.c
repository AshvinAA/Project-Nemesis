...
#undef DIR_ADD
	...
	for (i = 0; i < nb; i++)
	{
	    if (r < bag[i].w)
	    {
		mobjtype_t pick = bag[i].pool[P_Random () % bag[i].n];
		// JEV nemesis: if the player keeps killing a trash type quickly,
		// bias species selection toward the heavy pool.  Learned pressure
		// with zero model calls.
		extern const char* NEM_TypeNameOf (mobjtype_t);
		extern int NEM_Deaths (const char*);
		{
		    const char* tn = NEM_TypeNameOf (pick);
		    if (tn && NEM_Deaths (tn) >= 5)
			return dir_special1[P_Random () % (int)(sizeof(dir_special1)/sizeof(dir_special1[0]))];
							// escalate: over-farmed trash -> special tier
		}
		return pick;
	    }
	    r -= bag[i].w;
